...
$ cd /tmp/iam-${version}/ && ./scripts/install/install.sh iam::install::install
```
...
```bash
$ cd /tmp/iam-${version}/ && ./scripts/install/test.sh iam::test::test
```
...
$ export LINUX_PASSWORD='iam59!z$' # 重要！：这里要 export going 用户的密码
$ version=latest && curl https://marmotedu-1254073058.cos.ap-beijing.myqcloud.com/iam-release/${version}/iam.tar.gz | tar -xz -C /tmp/
$ cd /tmp/iam-${version}/ && ./scripts/install/install.sh iam::install::uninstall
```
