...
{
  rm -rf $WORKSPACE/golang/src/github.com/robinlg/iam # clean up

  # 1. 下载 iam 项目代码，先强制删除 iam 目录，确保 iam 源码都是最新的指定版本
  mkdir -p $WORKSPACE/golang/src/github.com/robinlg && cd $WORKSPACE/golang/src/github.com/robinlg
  git clone -b $(iam::install::obtain_branch_flag) --depth=1 https://github.com/robinlg/iam
  go work use ./iam

  # NOTICE: 因为切换编译路径，所以这里要重新赋值 IAM_ROOT 和 LOCAL_OUTPUT_ROOT
  IAM_ROOT=$WORKSPACE/golang/src/github.com/robinlg/iam
...
export IAM_ROOT="$GOSRC/github.com/robinlg/iam"
alias mm="cd $GOSRC/github.com/robinlg"
alias i="cd $GOSRC/github.com/robinlg/iam"
EOF
  fi
...
EOF
  ...
  # 6. 配置 hosts
  if ! egrep -q 'iam.*robinlg.com' /etc/hosts;then
    echo ${LINUX_PASSWORD} | sudo -S bash -c "cat << 'EOF' >> /etc/hosts
    127.0.0.1 iam.api.robinlg.com
    127.0.0.1 iam.authz.robinlg.com
    EOF"
  fi
...
EOF
  ...
  # 3. 删除配置 hosts
  echo ${LINUX_PASSWORD} | sudo -S sed -i '/iam.api.robinlg.com/d' /etc/hosts
  echo ${LINUX_PASSWORD} | sudo -S sed -i '/iam.authz.robinlg.com/d' /etc/hosts
